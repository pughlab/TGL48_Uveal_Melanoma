...
data$TF_sum <- ifelse(data$TF > data$TF_short, data$TF, data$TF_short)
...
### Import IchorCNA copy number segs
files <- list.files(path = ichorCNA, pattern = "*cna.seg", full.names = TRUE)
files_short <- list.files(path = ichor_short, pattern = "*cna.seg", full.names = TRUE)
...
### Format IchorCNA copy number segs and write data
data <- as.data.frame(data)
colnames(data) <- names
data <- data[ , colnames(data) %in% samples$sWGS]
data <- cbind(rows, data)
write.table(data, file.path(outdir, paste0(project, "_cna_segs.txt")), row.names = FALSE, sep = "\t")

data_short <- as.data.frame(data_short)
colnames(data_short) <- names_short
data_short <- data_short[ , colnames(data_short) %in% samples$sWGS]
data_short <- cbind(rows, data_short)
write.table(data_short, file.path(outdir, paste0(project, "_cna_segs_short.txt")), row.names = FALSE, sep = "\t")
